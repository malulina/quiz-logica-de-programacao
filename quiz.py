...
while True:
    ...
    if opcao == '1':
        ...
        for pergunta in perguntas:
            ...
            for i, alt in enumerate(pergunta['opcoes']):
                print(f"{i + 1}. {alt}")

            resposta = input("Digite o número da resposta ou 'sair' para encerrar o quiz: ")

            if resposta.lower() == 'sair':
                print(f"\nQuiz encerrado por {nome}. Pontuação parcial: {pontuacao} de {len(perguntas)}")
                break  


            try:
                indice = int(resposta) - 1
                if pergunta['opcoes'][indice] == pergunta['resposta']:
                    print("correto!")
                    pontuacao += 1
                else:
